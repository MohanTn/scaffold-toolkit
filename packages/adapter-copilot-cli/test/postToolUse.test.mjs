...
test('shouldCheckStatus is false for a non-shell tool call', () => {
...
test('shouldCheckStatus is true for a bash call whose command mentions scaffold generate directly', () => {
  assert.equal(shouldCheckStatus({ toolName: 'bash', toolArgs: { command: 'scaffold generate --manifest x.toon --force' } }), true);
});
...
test('shouldCheckStatus is true for a powershell call on Windows sessions', () => {
  assert.equal(shouldCheckStatus({ toolName: 'powershell', toolArgs: { command: 'scaffold generate --manifest x.toon' } }), true);
});

test('shouldCheckStatus parses toolArgs delivered as a JSON-encoded string', () => {
  assert.equal(
...
test('buildDecision surfaces the unresolved block list as a flat additionalContext when status is non-zero', () => {
  ...
  assert.equal(Object.prototype.hasOwnProperty.call(decision, 'decision'), false, 'postToolUse must nudge via additionalContext, not a blocking decision — the tool already ran');
});
