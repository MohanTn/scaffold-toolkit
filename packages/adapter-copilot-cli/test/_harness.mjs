/**
 * Shared fixture helpers for adapter-copilot-cli's integration tests.
 * Mirrors packages/adapter-claude-code/test/_harness.mjs's own mirror of
 * packages/core/test/integration/testHarness.ts — duplicating rather than
 * importing across a package boundary is intentional so this adapter's
 * tests don't carry a TS dependency on core's test code.
 *
 * Not picked up by `node --test test/*.test.mjs` (the underscore prefix and
 * the non-`.test.mjs` suffix keep it out of the runner's glob).
 */
...
export function buildFixturePackRepo() {
  const dir = mkdtempSync(path.join(tmpdir(), 'scaffold-ccli-pack-'));
  git(dir, ['init', '-q']);
...
export function buildFixtureTargetRepo() {
  const dir = mkdtempSync(path.join(tmpdir(), 'scaffold-ccli-target-'));
 ...
 * symlink: the CLI's bin entry needs `node` to run it, and a wrapper keeps
 * that explicit and portable across test runners.
 */
export function setupScaffoldOnPath(coreCliPath) {
  const binDir = mkdtempSync(path.join(tmpdir(), 'scaffold-ccli-bindir-'));
...
/** Run `node <scriptPath>` feeding `stdinObj` as JSON on stdin, always resolving with `{ stdout, stderr, status }`. */
