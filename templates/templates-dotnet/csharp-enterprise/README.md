...
- **Implicit `Id`**: every entity gets `public {{keyType}} Id`; manifest `fields` are business fields only (an explicit `Id` field is deduplicated). DTOs are positional records with `Id` first.
- **Configurable key type** (`options.keyType`, `add … --option keyType=<guid|int|long|string>`): defaults to `guid` (client-generated `Guid.NewGuid()`, `ValueGeneratedNever`). `int`/`long` switch to DB-generated identity (`ValueGeneratedOnAdd`, entity created with `Id = default`) and an `{id:int}`/`{id:long}` route constraint. `string` stays client-generated (`Guid.NewGuid().ToString()`) with no route constraint. `IRepository<T>` is generic over the key (`IRepository<T, TKey>`), so different entities in the same project may use different key types.
