...
**Refined in 0.2.0 — which blocks are "required".** The original rule tracked only blocks that shipped *empty*. But the base packs deliberately ship a compilable default inside each business-logic seam (so the generated code builds before the LLM touches it), which meant a plain `v8-controller`/`v10-minimal-api` project had *no* tracked blocks and `status` always passed — the enforcement never armed for the most common case. A pack now opts a block in explicitly by tagging its start marker with the reserved `:required` token (`SCAFFOLD:AI_IMPLEMENTATION:START:required`); such a block is tracked regardless of content, and resolves only when the agent changes it from the shipped placeholder. Empty blocks are still tracked implicitly. The pending record keys each block by its ordinal among all blocks in its file, not by line number (which drifts as earlier blocks are filled) or by position in the filtered pending list (only a subset of the file's blocks). A block's `required` flag is also surfaced in the generate report so a host agent knows which seams it must complete. `validatePack/validatePack.ts` backs a new `scaffold validate-pack` command that runs a real generate against a synthesized target repo, so a pack's injection wiring is exercised — not just its templates rendered — in CI.
