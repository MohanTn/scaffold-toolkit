...
test('buildIntentManifest rejects --input with reserved key collision', () => {
  const reserved = ['manifestSchemaVersion', 'targetStack', 'entity', 'fields', 'options', 'artifacts'];
...
test('buildIntentManifest carries --artifact tags into manifest.artifacts, omitted when empty', () => {
  const scoped = buildIntentManifest({
    targetStack: 'backend',
    entity: 'Invoice',
    fields: ['Amount:decimal'],
    options: [],
    inputs: [],
    artifacts: ['base', 'op-create'],
  });
  assert.deepEqual(scoped.artifacts, ['base', 'op-create']);

  const unscoped = buildIntentManifest({
    targetStack: 'backend',
    entity: 'Invoice',
    fields: ['Amount:decimal'],
    options: [],
  ...
  assert.equal('artifacts' in unscoped, false);
});
