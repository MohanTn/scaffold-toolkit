...
export function collectPackOwnership(config: ScaffoldConfig, repoRoot: string, cacheRoot: string): PackOwnershipMatcher[] {
  ...
  for (const [packSlot, pack] of Object.entries(config.packs)) {
    ...
    // Same pathConfig precedence as generate.ts: the slot's persisted
    // brownfield layout wins, the pack's own declared default fills in
    // otherwise — so a multi-segment fragment (e.g. "Api/Controllers")
    // resolves to its literal path instead of one single-segment wildcard.
    const context = { companyProjectName: pack.companyProjectName, pathConfig: pack.pathConfig ?? descriptor.pathConfig };
