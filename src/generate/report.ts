...
export interface GenerateReport {
  ...
  /** Echo of the manifest's `artifacts` selection (present when the manifest scoped the run) and how many descriptor entries the artifact/when filter skipped (present when nonzero) — see `generate/entryFilter.ts`. */
  artifacts?: string[];
  skippedEntries?: { byArtifact: number; byWhen: number };
