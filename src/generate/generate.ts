...
import { decodeManifestFile, validateManifest } from '../manifest/decode.js';
...
import { selectEntries } from './entryFilter.js';

/**
 * Exactly one of `manifestPath` (a .toon/.json file on disk — the classic
 * `--manifest` flow) or `manifest` (an already-built in-memory manifest —
 * the `scaffold add` compiler flow) must be provided. Both run through the
 * same schema validation and the same pipeline from there on.
 */
...
/**
 * Merges a pack slot's persisted `defaults` under an explicit manifest:
 * top-level manifest keys always win, and `options` merges one level deep
 * (an explicit `options.combine` beats `defaults.options.combine`, while an
 * unrelated `defaults.options.database.scope` still applies). Runs before
 * input validation so defaults can satisfy a pack-required input.
 */
export function applyPackDefaults(
  manifest: IntentManifest,
  defaults: Record<string, unknown> | undefined,
): IntentManifest {
  if (!defaults) return manifest;
  const defaultOptions = (defaults.options ?? {}) as Record<string, unknown>;
  const merged: IntentManifest = { ...defaults, ...manifest } as IntentManifest;
  merged.options = { ...defaultOptions, ...(manifest.options ?? {}) };
  return merged;
}
...
export async function runGenerate(options: GenerateOptions): Promise<GenerateReport> {
  ...
  if ((options.manifestPath === undefined) === (options.manifest === undefined)) {
    throw new Error('runGenerate needs exactly one of manifestPath or manifest');
  }
  const suppliedManifest =
    options.manifestPath !== undefined ? decodeManifestFile(options.manifestPath) : validateManifest(options.manifest);
  const config = loadConfig(repoRoot);

  const pack = config.packs[suppliedManifest.targetStack];
  if (!pack) {
    throw new Error(`no pack configured for targetStack "${suppliedManifest.targetStack}" — check .scaffold/config.json`);
  }

  const manifest = applyPackDefaults(suppliedManifest, pack.defaults);
  ...
  // pathConfig precedence: manifest-level (spread inside buildHandlebarsContext)
  // > pack slot's persisted brownfield layout > the pack's own declared default.
  const context = buildHandlebarsContext(manifest, {
    pathConfig: pack.pathConfig ?? descriptor.pathConfig,
    companyProjectName: pack.companyProjectName,
  });

  // --- Artifact/when selection (see entryFilter.ts) --------------------------------
  // A filtered-out entry is invisible to everything downstream: a skipped
  // create-mode target can't throw "already exists", a skipped injection is
  // never rendered. A manifest without `artifacts` selects everything.
  const targetSelection = selectEntries(descriptor.targets, manifest.artifacts, context);
  const injectionSelection = selectEntries(descriptor.injections, manifest.artifacts, context);
  ...
  for (const target of targetSelection.selected) {
  ...
  for (const injection of injectionSelection.selected) {
    ...
    group.requests.push({
      ...
      hashTrailerSuffix: injection.hashTrailerSuffix,
  ...
  if (manifest.artifacts !== undefined) report.artifacts = manifest.artifacts;
  const skippedByArtifact = targetSelection.skippedByArtifact + injectionSelection.skippedByArtifact;
  const skippedByWhen = targetSelection.skippedByWhen + injectionSelection.skippedByWhen;
  if (skippedByArtifact > 0 || skippedByWhen > 0) {
    report.skippedEntries = { byArtifact: skippedByArtifact, byWhen: skippedByWhen };
  }
