...
export interface InjectionRequest {
  ...
  /** Closes the trailer line for wrap-style comment syntaxes (e.g. ` -->`); see descriptor/schema.ts. */
...
/** Splits an interior's lines into {existing hash trailer hex, remaining content lines}, or no hex if no trailer line is present. A trailer suffix (wrap-style comment close, e.g. ` -->`) is stripped before the hex is read. */
function extractExistingHash(interiorLines: string[], hashTrailerPrefix: string, hashTrailerSuffix?: string): { hex?: string; contentLines: string[] } {
  ...
  let rest = interiorLines[trailerIdx].trim().slice(hashTrailerPrefix.length);
  if (hashTrailerSuffix !== undefined && rest.trimEnd().endsWith(hashTrailerSuffix.trimEnd())) {
    rest = rest.trimEnd().slice(0, -hashTrailerSuffix.trimEnd().length);
  }
  const hex = rest.trim();
...
export function injectMarkers(filePath: string, originalContent: string, requests: InjectionRequest[], force: boolean, freshlyCreated = false): InjectResult {
  ...
  for (const request of requests) {
    ...
    const { hex: existingHex, contentLines: existingContentLines } = extractExistingHash(interiorLines, request.hashTrailerPrefix, request.hashTrailerSuffix);
    ...
    if (request.strategy === 'append' && !isEmpty) {
      ...
      const appendTrailerLine = `${request.hashTrailerPrefix}${contentHash(accumulatedText)}${request.hashTrailerSuffix ?? ''}`;
    ...
    const trailerLine = `${request.hashTrailerPrefix}${newHash}${request.hashTrailerSuffix ?? ''}`;
