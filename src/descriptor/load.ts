 ...
 * schema, plus a range-check of `requires.scaffoldCli` against the installed
 * CLI's own version (shape is ajv's job, the semver range check is
 * separate) — failing fast before any file is touched.
 *
 * The requires check runs FIRST, straight off the raw JSON: a pack authored
 * for a newer CLI may carry entry fields this schema rejects
 * (additionalProperties:false per entry), and the user-actionable error in
 * that situation is "your CLI is too old", not a schema-error wall.
 */
...
export function loadDescriptor(descriptorPath: string, installedVersion?: string): TemplateDescriptor {
  ...
  // Version gate before shape validation: see module doc.
  const requires = (parsed?.requires ?? {}) as Record<string, unknown>;
  const requiredRange = typeof requires.scaffoldCli === 'string' ? requires.scaffoldCli : undefined;
  if (requiredRange && semver.validRange(requiredRange)) {
    ...
    if (!semver.satisfies(version, requiredRange, { includePrerelease: true })) {
      const packVersion = typeof parsed.packVersion === 'string' ? parsed.packVersion : '(unknown)';
      throw new DescriptorRequiresMismatchError(
        `pack "${packVersion}" requires scaffoldCli ${requiredRange}, but the installed CLI is v${version}`,
      );
    }
  }
