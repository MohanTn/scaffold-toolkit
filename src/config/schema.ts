...
interface PackConfigCommon {
  ...
  /**
   * Free-form manifest defaults merged UNDER every manifest driving this
   * slot (`generate.ts`'s `applyPackDefaults`): explicit manifest keys win,
   * `options` merges one level deep. Spec-style config ergonomics live here
   * (e.g. `defaults.options.combine`, `defaults.options.database.scope`)
   * instead of a parallel config vocabulary.
   */
