...
export interface IntentManifest {
  ...
  /**
   * Which artifact tags to render (see `generate/entryFilter.ts`). Absent ⇒
   * every descriptor entry renders, exactly the pre-tag behavior; present ⇒
   * only entries whose `artifact` tag (untagged ⇒ `base`) is listed.
   */
  artifacts?: string[];
  [extra: string]: unknown;
}
