...
import { selectEntries } from '../generate/entryFilter.js';
...
export async function validatePackVersion(packDir: string, version: string, manifestPath: string): Promise<PackValidationResult> {
  ...
  try {
    ...
    const context = buildHandlebarsContext(
      manifest as unknown as { entity?: string; fields?: unknown; options?: Record<string, unknown> },
      { pathConfig: descriptor.pathConfig },
    );

    // Mirror runGenerate's artifact/when selection: a filtered-out injection
    // is never exercised, so its host file must not be synthesized either
    // (an artifact-scoped pack's unselected injections may reference inputs
    // this manifest doesn't carry).
    const selectedTargets = selectEntries(descriptor.targets, manifest.artifacts, context).selected;
    const selectedInjections = selectEntries(descriptor.injections, manifest.artifacts, context).selected;
    ...
    const createdPaths = new Set(selectedTargets.map((t) => renderPathTemplate(t.output, context)));
    const injectionsByFile = new Map<string, DescriptorInjection[]>();
    for (const injection of selectedInjections) {
