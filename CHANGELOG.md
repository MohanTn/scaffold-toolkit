...
- This change implements a PreToolUse gate (`scaffold check-edit` CLI command plus adapter hooks) that blocks AI agents from directly writing or editing files owned by template packs, enforcing the workflow of going through `scaffold generate`. It also adds a manual-only benchmark harness that measures token usage, cost, and build success when adding a feature with scaffold versus freehand authoring.
